...
def search(domain, limit):
    all_emails = []
    app_retrieveemail.show_message("[+] Searching in Twitter")
    ...
    app_retrieveemail.init_search(yahooUrl, domain, limit, 1, 100, 'Yahoo + Twitter')
    ...
    app_retrieveemail.init_search(bingUrl, domain, limit, 0, 50, 'Bing + Twitter')
    app_retrieveemail.process()
    all_emails += app_retrieveemail.get_emails()
    
    googleUrl = 'https://www.google.com/search?num=100&start={counter}&hl=en&q=site%3Atwitter.com+intitle:"on Twitter"+"%40{word}"'
    app_retrieveemail.init_search(googleUrl, domain, limit, 0, 100, 'Google + Twitter')
    app_retrieveemail.process()
    all_emails += app_retrieveemail.get_emails()

    url = 'http://www.baidu.com/search/s?wd=site%3Atwitter.com+intitle:"on Twitter"+"%40{word}"&pn={counter}'
    app_retrieveemail.init_search(url, domain, limit, 0, 10, 'Baidu + Twitter')
    app_retrieveemail.process()
    all_emails += app_retrieveemail.get_emails()
    ...
    app_retrieveemail.init_search(url, domain, limit, 0, 50, 'Exalead + Twitter')
    app_retrieveemail.process()
    all_emails += app_retrieveemail.get_emails()
